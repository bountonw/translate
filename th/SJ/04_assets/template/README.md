...
└── 04_assets/
    ...
    │   ├── config.typ          # Leaf: metadata, page sizing, header/footer, typography (fonts, sizes, leading)
    │   ├── styles.typ          # Show rules (headings, paragraphs, poetry, …) + apply-styles
...
**Module layering.** Imports flow one way up the stack: `config.typ` and
`dictionary.typ` are leaves (constants, page setup, word lists); `components.typ`
imports only from them; `styles.typ` sits above and may import any component (e.g.
it pulls in `EGW` so the ` ```poetry ` block's `eval` can resolve the tag in its
scope); `lib.typ` re-exports everything. Keeping shared constants in the leaf
`config.typ` is what lets `styles → components` exist without a circular import.
...
| Header | verso: `page – Book Title` · recto: `*บทที่ N* Title – page` (right) | same on every body page: `Book Title` (left) · `updated date` (center) · `*บทที่ N* Title` (right) |
...
The proof header's centered date comes from `apply-styles`' `updated:` argument,
e.g. `#show: apply-styles.with(proofing: true, updated: "17 มิถุนา 2026")`. It
is shown only in proof mode; omit it and the center stays blank.
...
Typography constants also live here, in the leaf module, so `components.typ`
can reference them (e.g. `font-heading`) without importing `styles.typ`:
...
Document-wide show rules plus `apply-styles`. Heading sizes/weights, footnote
styling, and poetry formatting live here in the `heading-styles`,
`footnote-styles`, and `poetry-styles` show-rule blocks. The typography
constants used by these rules are imported from `config.typ` (see above).
...
| Fonts | `config.typ` | `font-body`, `font-heading` |
| Body / footnote sizes | `config.typ` | `size-body`, `size-small` |
| Leading per mode | `config.typ` | `book-leading`, `proof-leading` |
