...
1.B. The measurement window is git. HEAD holds the clean pre-run chapter; the working tree holds the resolved one. Scope both commands to the chapter and name HEAD in both, because a bare git diff compares the working tree against the index and returns nothing once the resolutions are staged: git diff -U0 HEAD -- lo/GC/03_public/GCNN_lo.md lists the changed lines (one line is one paragraph), and git diff --word-diff HEAD -- lo/GC/03_public/GCNN_lo.md locates the splice points within them. Read-only git commands only.
...
3.B. ASCII letters embedded in Lao text outside parentheses. Parenthetical romanizations such as (Menno Simons) are an established convention and are not defects, and neither is any LaTeX macro of the typesetting pipeline — \s and \S (its flex and rigid space markers), {\;}, \thai{...} around Thai script, or another backslash code. A mangled macro is the one exception: a backslash or brace lost so that stray letters sit in the Lao text is an introduced typo and a defect. This is the translator's ruling of 26 August.
...
The detail section comes first and carries the same items in the same order as the summary list beneath it, each with a heading that states its point in one sentence and a labelled block under the heading:
