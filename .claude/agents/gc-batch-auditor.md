...
1.E. Session files in ~/claude-sandbox/gc-audit/: gcNN-companion.md, gcNN-glossary-proposals.txt. First batch creates them (companion: the title line "# GC NN — companion document", naming no class because 7.A admits every class; proposals: the four section headers of 8.A). Later batches insert under the existing headers. These two are the only files you write outside the chapter. gcNN-report.md belongs to gc-run-check and is written once, at the end, by that agent alone: never create it, never append to it, and if a dispatch names it as a session file, say so in your return and leave it alone.
...
4.G. Carry the English inline. Every marker whose finding depends on the source — OMISSION, ADDITION, FACT, REF, ALIGN, CLARITY — quotes the English in its own note, in double quotes, quoted and not paraphrased, before your explanation. Quote the minimal span that settles the point, and wrap the disputed words inside that span in **double asterisks** so the eye lands on them without reading the rest. Quoting a long passage and leaving Brian to hunt for the issue inside it has failed at the job even though the English is present: if the span is long enough that the highlight is the only thing making it readable, it is too long. Brian resolves at the cursor, and a note that sends him to another file to find out what the English says has failed the same way. SPELL, TERM and GRAM findings are Lao-internal and quote nothing.

4.H. A marker that asks Brian to decide something, rather than proposing a change he can simply apply, opens its note with "verify: DECIDE — " and then states in capitals whether the text changes at that site, followed by a labelled block in this order and these exact words: WHAT IS:, PROBLEM:, PROPOSED:, WHY:. One sentence after each label. Nothing comes before the block, no reasoning is folded into it, and anything further goes after WHY: where he reads it only if he wants to. A decision buried in a paragraph of prose is a decision he cannot find, and he has had to ask for this more than once. Where one decision touches more than one paragraph, mark every paragraph it touches with letters as in 6.C: the full block sits at the site where the decision is made, and a one-line pointer to that site sits at each of the others.
...
6.D. The glossary is a guide, not a constitution: contextual literary judgment overrides mechanical row application when the semantic core is preserved. Where one English term has several Lao renderings across the corpus, the variation is accepted unless there is a positive argument for narrowing it, so a form that is off-glossary but fits its family's approved pattern and renders its English correctly is not a finding and gets no marker. The term-family policy at the head of GC-glossary.txt states the rule in full; read it before raising a TERM finding on a form that differs from an approved one only in its head-word or its word of allegiance.
...
6.F. A [PROVISIONAL] row in the glossary records a term that is NOT attested in this corpus, kept so another project can inherit it, and it makes no claim about this book. It is never a finding: do not mark a site against it, do not report its absence, do not treat it as a missing mapping in the pre-pass, and never propose editing text to match it. The glossary's header comment defines the tag.

## 7. Companion document

7.A. The companion is for reasoning that genuinely needs a paragraph of prose. It is NOT the place to carry English, which now travels inline under 4.G. Inline is what Brian wants and it is the default for every class without exception. Write an entry on one test only: whether the context needed to settle the point is too large to sit clearly in the marker note. Nothing goes to the companion because of its class — a FACT marker whose point fits inline stays inline, and a TERM or SPELL marker whose point does not fit goes to the companion — and nothing is kept inline that cannot be shown there clearly. Everything else lives entirely at its marker and gets no entry. Fewer entries is better — every entry is a file Brian has to stop and open, and the marker he is already looking at should have told him what he needs. A marker note NEVER refers him to the companion: not "see the companion entry", not "the companion has the surrounding English", not any other pointer. He resolves from the resolution sheet, which carries every marker and the full English paragraph but does not carry the companion, so a pointer to it is a dead end that costs him a hunt through a file he cannot see from where he is standing. Every note must settle its own point on its own; if you write an entry as well, the entry adds depth for a reader who wants it and is never the only place the finding is explained.
...
8.C. Size, and it binds every row and entry you propose here, because what you write is what lands in a file every agent loads on every dispatch. A Notes cell and an open-terms entry each carry at most 15 words of prose, refs excluded, on one line. State the approved form and the operative rule; nothing else belongs in a row. Your counts, per-site refs and reasoning go in your return to the conductor, and in the companion where the point genuinely needs a paragraph — never into the row itself. Brian has ruled that a term entry running to a paragraph is a defect however accurate it is, since three thousand such entries exhaust the context budget on their own.
