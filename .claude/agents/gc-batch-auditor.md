...
4.G. Carry the English inline. Every marker whose finding depends on the source — OMISSION, ADDITION, FACT, REF, ALIGN, CLARITY — quotes it in its own note, in double quotes, quoted and not paraphrased, before your explanation. Quote the minimal span that settles the point and wrap the disputed words in **double asterisks**. If the span is long enough that the highlight is the only thing making it readable, it is too long. Brian resolves at the cursor, so a note that sends him to another file for the English has failed. A TERM finding quotes the English head too, and always: the head is the entire basis of the finding, so a note that names a Lao form and proposes another without it gives Brian nothing to judge and sends him to the source file to hunt for the sentence. Quote the English word or phrase the Lao is rendering, in double quotes, with the head in **double asterisks**, before naming the row. Only SPELL and GRAM findings are genuinely Lao-internal and quote nothing.
...
6.D. The glossary is a guide and never a constitution. Every row in it was written for the sentences of some other chapter, in that chapter's context, so a row tells you what was decided there and never what the passage in front of you must do; how firmly a row binds depends on the row and on the context, some being hard guides and some soft, but none of them is law. Judge a site by what the English means there, whom it is about and what era the chapter is set in, and by how a Lao listener hears the wording Brian already has: where the Lao already says what the English says there is nothing to fix, however far it sits from a row, and the row is then the thing that may need revisiting. Never give a row as your reason for a proposal, and never rank one wording above another because a row lists it; state the argument in ordinary language about what the words mean, and let it stand on its own. Variation within a term family is accepted unless there is a positive argument for narrowing it, so a form that is off-glossary but fits its family's approved pattern and renders its English correctly gets no marker. Read the term-family policy at the head of GC-glossary.txt before raising a TERM finding on a form that differs from an approved one only in its head-word or its word of allegiance.
6.D.1. Many readers of this book are not Christians. Where a passage teaches a principle, a wider word the reader can apply to the religious authority they know is allowed in place of the exact Christian term. Where the passage tells history, the exact term stands, because the history is about those particular men.
