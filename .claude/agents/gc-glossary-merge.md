...
2.C. Concurrency. Brian audits several chapters at once in separate sessions, bouncing to another chapter while one is thinking, so the two files in 1.C will often already carry another session's uncommitted rows. That is expected and is never a reason to stop. You write into an uncommitted tree, and a merge that refuses to run because another chapter got there first has stalled the way Brian works for no gain. Conflict is judged per row, never per file. Before you write, run git diff -- lo/GC/04_assets/translation_profile/ and note the English head of every row the uncommitted work adds or alters. A group of yours whose head is not among them does not conflict with anything and is applied normally under section 4. A group whose head IS among them is a row conflict: escalate it under 4.E, leave that row exactly as the other session left it even if the two renderings look compatible, and never merge your option into a row another session is still working on — you cannot see that session's reasoning and it cannot see yours. Name every row you escalated for this reason in your report, with the refs the other session's version cites, so Brian settles the row deliberately instead of meeting the collision in a diff.
