...
2.C. Concurrency. The 1.C files usually carry another session's uncommitted rows, because Brian audits several chapters at once. Never stop for that: you write into an uncommitted tree. Conflict is per row, never per file. First run git diff -- lo/GC/04_assets/translation_profile/ and list the English heads the uncommitted work adds or alters. Apply your groups normally under section 4 unless a head matches one of those; that row is a conflict, so escalate it under 4.E, leave it exactly as the other session left it even if the two renderings look compatible, and report it with the refs the other version cites.
...
3.D. After writing, run git diff -- lo/GC/04_assets/translation_profile/ and read it. Confirm every hunk is either one you intended or one you already identified in 2.C as another session's uncommitted work, that no existing row lost content, and that no line you did not mean to touch moved. If the diff contains anything you cannot account for under one of those two headings, say so in the report rather than attempting a repair.
3.E. Write the report per section 6.
3.F. Size gate, applied to every row and entry before you write it, and you are the last gate before these files. A Notes cell carries at most 15 words of prose, refs excluded, on one line — the same limit the batch auditors work under in their 8.C. A proposal over the limit is not escalated for that reason alone: cut it down to the approved form and the operative rule, which is the whole of what a row is for, and record in the merge report what you cut so Brian can see what was dropped and put it elsewhere if he wants it. Counts, per-site refs and reasoning belong in your report under ~/claude-sandbox/gc-audit/, which no agent loads, and never in a glossary row or a GC-open-terms.md entry. GC-open-terms.md is NOT an overflow home: gc-batch-auditor 2.B requires reading it for every ref in a batch's range, so it is loaded on every batch dispatch exactly as the glossary is, and moving bulk into it relocates the cost instead of removing it. Every agent on every dispatch loads both files, so a paragraph-length row or entry is a permanent tax on every future run.
...
4.K. Size, and it is a hard limit rather than a preference. Every row and every open-terms entry you write carries at most 15 words of prose in its Notes, refs excluded, and stays on one line. Every agent in this audit loads these two files on every dispatch, so an entry's length is a cost the whole run pays, and at three thousand terms the habit of writing a paragraph per term spends the context budget before a chapter is read. Write the operative instruction and stop: the approved form, the form that is wrong, and "never mark" where a family is closed. Corpus counts, per-site ref lists, your reasoning and the history of the decision are evidence, and evidence goes in gcNN-merge-report.md, which no agent loads. A row that is accurate and long has failed; Brian has said so of rows written exactly that way.
