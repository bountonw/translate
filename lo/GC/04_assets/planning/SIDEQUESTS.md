...
QA3 findings for the rework, one line per chapter as they land: GC12 {GC 233.2} — the Council row at `lo/GC/04_assets/translation_profile/GC-glossary.txt:96` lists two forms, ສະພາຄຣິສຕະຈັກ and ປະຊຸມສະພາ, with no note on which applies where; the QA3 REVERT there chose the general ປະຊຸມສະພາ over the church-specific form because the decree the paragraph names is civic, so the row wants a usage note distinguishing the two rather than leaving them interchangeable.

GC12 {GC 230.2}/{GC 230.3} — the translator's own romanized gloss "(Reign of Terror)" moved from the term's second mention to its first; the gloss is not a glossary-row term at all, so the rework should confirm whether first-mention placement for a translator's own romanized gloss belongs in the glossary system this entry is designing or stays a case-by-case style choice outside it.

GC01 {GC 22.2} — the English title "The Majesty of heaven" has no glossary row and diverges across the book: GC01 {GC 22.2} reads ອົງຜູ້ຊົງສະຫງ່າຣາສີແຫ່ງຟ້າສະຫວັນ (QA3 upheld it), GC39 {GC 617.1} reads ພຣະເຈົ້າແຫ່ງຟ້າສະຫວັນ, and GC40 {GC 651.2} restructures the sentence and carries no title; the rework decides whether a row is wanted and which form it fixes.

GC02 {GC 39.2} and {GC 43.1} — two in-context confirmations of the [CHECK]-flagged row "Paganism (the system) | ລັດທິນອກສາສະໜາ" at GC-glossary.txt:50: both passages speak of the pagan religious system as such, and the same paragraphs keep ຄົນທີ່ຂາບໄຫວ້ຮູບເຄົາລົບ for "worshipers of idols", so the two families stay distinct; evidence toward clearing the CHECK when the rework weighs the row.

GC04 {GC 64.1} — the prelate row's bare form ຜູ້ນຳຄຣິສຕະຈັກ (GC-glossary.txt:30) is ambiguous where the passage also holds the true church's own leaders: at {GC 64.1} the English "pope or prelate" sits two sentences after "the leaders of these churches" meaning the Waldensian leaders (ຜູ້ນຳຂອງຄຣິສຕະຈັກເຫຼົ່ານັ້ນ), and the QA2 form ພວກຜູ້ນຳຄຣິສຕະຈັກຝ່າຍໂຣມ stands precisely because it departs from the bare form; the rework should consider licensing the tail ຝ່າຍໂຣມ where both parties' leaders share a paragraph.
...
GC03 {GC 56.1} — QA2's monk correction makes GC03 the book's first ນັກບວດ site, ahead of GC 120.3 where the row at GC-glossary.txt:19 places the term's first-use footnote; the rework decides whether the footnote moves to {GC 56.1}.
...
GC06 {GC 114.3} and {GC 107.2} — the Church dignitaries row at GC-glossary.txt:215 fixes ເຈົ້າໜ້າທີ່ຂອງຄຣິສຕະຈັກ, but QA3 marker #4 proposes the pre-QA ຜູ້ນຳຄຣິສຕະຈັກ at {GC 114.3}, where the English "dignitaries of the church" means senior churchmen offering brilliant prospects and ເຈົ້າໜ້າທີ່ reads as functionaries, while {GC 107.2} kept ເຈົ້າໜ້າທີ່ only because ຜູ້ນຳຄຣິສຕະຈັກ already stood two clauses away in the same sentence; the rework should consider licensing ຜູ້ນຳຄຣິສຕະຈັກ for "dignitaries" where rank is in view and no prelate word collides.
