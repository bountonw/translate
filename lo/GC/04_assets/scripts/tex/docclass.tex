...
% Page dimensions and margins for B5-ish format
\usepackage[
  paperwidth=176mm,
  paperheight=250mm,
  top=14mm,
  inner=16mm, % alias of left/lmargin
  outer=14mm, % alias of right/rmargin
  bottom=14mm, % bottom margin of the page
  bindingoffset=6mm, % bindingoffset + inner = distance from inside of book (center) to where text starts (21mm)
...
% Latin letters of a Bible version code, sized to sit level with the Lao body text.
% Used only where a manuscript writes it (the GC00 introduction's version list).
\newcommand{\GCcode}[1]{{\fontsize{9.5pt}{12.6pt}\selectfont #1}}
...
\newcommand{\bookauthor}{ເອເລນ ຈີ. ໄວທ໌}
